...
public class LSelectPurchase implements Listener{
	
	public static Market plugin;
	public LSelectPurchase(Market instance){
		 plugin = instance;
		}
	
	@EventHandler(priority = EventPriority.NORMAL)
	public void onRightClickSneaking(PlayerInteractEvent event){
		...
		if (!(a.equals(Action.RIGHT_CLICK_BLOCK))  || !(event.getPlayer().isSneaking())) return;
		    ...
		    MessageUtil message = new MessageUtil(MessageType.PURCHASE_SELECTED);
		    message.send(p);
		    
		    Sign s = (Sign) event.getClickedBlock().getState();
		    if (Market.player_purchase.containsKey(p)){
		    	Market.player_purchase.remove(p);
		    	Market.player_purchase.put(p, s);
		    	return;
		    }
		    Market.player_purchase.put(p, s);
